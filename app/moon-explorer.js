/* ==========================================================================
   Ari & Dot Book 1 — Moon Explorer runtime (pages M1-M10)
   --------------------------------------------------------------------------
   A page says what it is and nothing else:

       <body data-mx="hub">                     M1
       <body data-mx="family" data-family="jupiter">   M2-M8
       <body data-mx="fame">                    M9
       <body data-mx="game">                    M10

   Everything else -- which moons, in what order, what they orbit, what they are
   known for, which ones the game asks about -- comes from data/moons.json. Add a
   moon there and it appears on its family page, in the quiz, and (if it has a
   `fame`) in the Hall of Fame, without any page being edited.

   Sound: a moon whose name the book has already recorded plays that clip. The
   other thirty-nine are silent and show a written pronunciation instead. There
   is deliberately NO speechSynthesis fallback -- see data/moon-narration.json.
   ========================================================================== */
(function () {
  "use strict";
  ...
  function moonsOf(fam) { return DATA.moons.filter(function (m) { return m.family === fam; }); }
  function byId(id) { return DATA.moons.filter(function (m) { return m.id === id; })[0]; }
  function famOf(id) { return DATA.families.filter(function (f) { return f.id === id; })[0]; }
  function hostsOf(fam) {
    var seen = [], out = [];
    moonsOf(fam).forEach(function (m) {
      if (seen.indexOf(m.orbits) < 0) { seen.push(m.orbits); out.push(m.orbits.toLowerCase()); }
    });
    return out;
  }
  ...
  function bar() {
    ...
    el.className = "mx-bar";
    el.setAttribute("aria-label", "Moon Explorer navigation");
    var left = MODE === "hub"
      ? '<a class="mx-btn" href="page-20.html">‹ Back to the story</a>'
      : '<a class="mx-btn" href="page-m1.html">‹ Moon Explorer</a>';
    el.innerHTML =
  ...
  function makeSheet() {
    ...
    sheet.setAttribute("aria-label", "About this moon");
      ...
      '<div class="in"><div>' + art(moon.avatar) + '</div><div>' +
        ...
        '<div class="mx-acts">' +
          ...
          (opts.family !== false && MODE !== "family"
            ? '<a class="mx-pill" href="page-' + famOf(moon.family).page + '.html">All of ' +
              esc(famOf(moon.family).title.replace(/^The /, "")) + " ›</a>" : "") +
        "</div>" +
  ...
  function renderHub() {
    ...
    head.innerHTML =
      '<div class="mx-head"><div class="mx-hosts">' + art("jupiter") + art("saturn") + "</div>" +
      '<div class="mx-titles"><h1>Moon Explorer</h1>' +
      '<p class="sub">A little atlas of the moons — go wherever you like</p></div></div>' +
      '<p class="mx-says mx-intro-says"><b>Ari:</b> “Dot, how many moons are there?”<br>' +
      '<b>Dot:</b> “Wrong question. The good question is <em>whose</em> moon is it — ' +
      'and what is it famous for. Pick a family and find out.”</p>';
    document.body.appendChild(head);

    var menu = document.createElement("div");
    menu.className = "mx-menu";
    menu.innerHTML = DATA.families.map(function (f) {
      var pics = hostsOf(f.id).slice(0, 2).map(function (h) { return art(h); }).join("");
      ...
      '<a class="mx-card special" href="page-m9.html"><span class="pics">' + art("ganymede") +
        '</span><span><span class="t">Moon Hall of Fame</span>' +
        '<span class="s">The one each moon is famous for</span></span></a>' +
      '<a class="mx-card special" href="page-m10.html"><span class="pics">' + art("triton") +
        '</span><span><span class="t">Whose Moon?</span>' +
        '<span class="s">A game — can you say what it goes round?</span></span></a>';
  ...
  function renderFamily() {
    var f = famOf(FAMILY);
    if (!f) return;
    document.title = "Ari & Dot · Moon Explorer — " + f.title;
    var all = moonsOf(FAMILY);
    ...
    head.innerHTML =
      '<div class="mx-head"><div class="mx-hosts">' +
        hostsOf(FAMILY).map(function (h) { return art(h); }).join("") + "</div>" +
    ...
    if (rest.length) {
      ...
      lab.textContent = "More moons to know by sight";
  ...
  function renderFame() {
    var head = document.createElement("div");
    head.innerHTML =
      '<div class="mx-head"><div class="mx-hosts">' + art("ganymede") + art("io") + "</div>" +
      '<div class="mx-titles"><h1>Moon Hall of Fame</h1>' +
      '<p class="sub">Every one of these is the best in the solar system at something</p>' +
      "</div></div>" +
      '<p class="mx-intro">Tap any of them to find out more, or to jump to the rest of ' +
      "its family.</p>";
    document.body.appendChild(head);

    var g = document.createElement("div");
    g.className = "mx-fame";
    g.innerHTML = DATA.hallOfFame.map(function (id) {
      var m = byId(id);
  ...
  function renderGame() {
    ...
    wrap.innerHTML =
      '<div class="mx-head" style="padding:0"><div class="mx-titles">' +
        "<h1>Whose Moon?</h1>" +
        '<p class="sub">Say which world it goes around</p></div></div>' +
    ...
    function start() {
      ...
      pool = DATA.moons.filter(LEVELS[level].pick);
    ...
    function next() {
      ...
      // Wrong answers are other real parents, so every option is plausible.
      var others = [];
      ...
      wrap.querySelector("#mxAnswers").innerHTML = opts.map(function (o) {
        return '<button class="ans" type="button" data-ans="' + esc(o) + '">' +
          art(o.toLowerCase()) + "<span>" + esc(o) + "</span></button>";
      }).join("");
  ...
  Promise.all([
    ...
    document.body.insertAdjacentHTML("beforeend",
      '<p class="mx-intro">The moon list could not be loaded. ' +
